import { MessageStatus, type SharedMessages, type TreeNode } from "@kono/models";
import { integer, sqliteTable, text, numeric, index, uniqueIndex } from "drizzle-orm/sqlite-core";
...
export const messages = sqliteTable("messages", {
  ...
  status: text("status", { enum: ["ungenerated", "in_progress", "completed", "aborted", "error"] }).notNull(),
...
export const sharedChats = sqliteTable("shared_chats", {
  ...
  title: text("title"),
  messagesJson: text("messages_json", { mode: "json" }).$type<SharedMessages>().notNull(), // stores messages as JSON
...
export const treeNodes = sqliteTable("tree_nodes", {
  ...
  chatId: text("chat_id").notNull().references(() => chats.id),
  childrenJson: text("children_json", { mode: "json" }).$type<TreeNode>().notNull(), // stores children as JSON
